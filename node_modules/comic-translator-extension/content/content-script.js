class ComicTranslator {
  ...
  async loadSettings() {
    ...
    this.settings = {
      apiUrl: settings.apiUrl || 'https://Inkverta-api.up.railway.app',
    ...
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
      ...
      if (request.action === 'translate-page') {
        ...
        if (this.selectedImage?.src) {
          this.translateImage(this.selectedImage.src, this.selectedImage);
        } else {
          this.displayTranslation(
            'Tip',
            'Right-click an image then use “Translate Image”, or double-click a comic image.',
          );
        }
      }
      if (request.action === 'translate-selection') {
        this.translateSelection(request.text || '');
      }
    });
  }
  ...
  async performTranslation() {
    if (this.selectedImage?.src) {
      return this.translateImage(this.selectedImage.src, this.selectedImage);
    }
    this.displayTranslation('No image selected', 'Right-click an image first.');
    return undefined;
  }
  ...
  async translateSelection(text) {
    try {
      ...
      if (!trimmed) {
        this.showOverlay();
        this.displayTranslation('No text selected', 'Select text on the page and try again.');
        return;
      }
      ...
      this.displayTranslation(trimmed, result.translatedText);
    } catch (error) {
